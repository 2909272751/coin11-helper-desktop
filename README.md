...
- **界面与图标**：现代浅色“毛玻璃感”单页运行台（半透明白卡、柔和渐变背景、
  圆角、青绿强调）+ 品牌图标（`assets/coin11-logo-v1.png` 为源，构建生成
  `.ico`，均不覆盖源文件）；任务清单为窗口弹性主体，日常操作集中在单页；
- **维护与更新菜单**：下载中心、重新检测组件、运行时目录、同步/回退脚本、
  打开日志目录等低频维护项收纳于“维护与更新 ▾”菜单；“设备 ▾”收纳重新
  接管 ADB 与连接说明；实时日志默认折叠为状态条、可展开。
